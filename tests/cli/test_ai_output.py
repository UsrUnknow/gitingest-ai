...
def test_ai_output_python_content(tmp_path, max_file_size, n_files, file_size):
    ...
    # Les fichiers doivent être générés dans le répertoire courant (tmp_path)
    output_files = sorted(Path.cwd().glob(f"*_{model}_part*.txt"))
    assert output_files, "Aucun fichier output généré"
